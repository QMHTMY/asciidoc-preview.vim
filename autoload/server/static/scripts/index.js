(function(global) {
    var TOTOPHEIGHT = 100;
    var asdID, body, html, asdContainer, options, isLoadMathJax;

    asdID = window.location.pathname.split('/').slice(2);
    body = document.body;
    html = document.querySelector('html');
    asdContainer = document.getElementById('js-asciidoc');
    ...
    function openConn() {
        global.conn = new WebSocket('ws://' + window.location.host + '/WebSocket/' + asdID);
    ...
    function onMessage(event) {
      ...
      if (data === '[019600976811CE18D7D4F7699D774DFF](close)') {
        ...
        asdRefresh(event.data);
      }
    }

    function asdRefresh(data) {
        asciied(data, function(err, content) {
            var aPoint;
            if(err) {
                throw err;
            }
            asdContainer.innerHTML = content;
