...
class Util():
        ...
        if paths[1] == u'WebSocket':        #shakehand of websocket
            for header in hItmes[1:]:
                ...
                if key == u'Sec-WebSocket-Key':
                    ...
                    break
        elif paths[1] == u'asciidoc':       #get asciidoc preview page
            f = open(u'./static/htmls/index.html', u'r')
            ad = f.read()
            if str(type(ad)) == "<class 'str'>":        #Compatible with python3
                ad = ad.encode(ENCODING)
            f.close()
            conn.send((MD_HEADER % (len(ad), MIME_TYPE[u'html'])).encode(ENCODING) + ad)
            conn.close()
    ...
    @staticmethod
    def doPut(conn, hItmes, body, path):        #refresh the asciidoc preview page
        for header in hItmes[1:]:
            key, val = header.split(u': ')
    ...
    @staticmethod
    def doDelete(conn, path):       #close the asciidoc preview page
        conn.send(OK_HEADER.encode(ENCODING))
        conn.close()
        bufnr = path[1:]
        if bufnr in sockets:
